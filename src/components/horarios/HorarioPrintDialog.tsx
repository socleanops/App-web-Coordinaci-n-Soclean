    ...
    const handlePrint = () => {
      ...
      <html>
        ...
        sortedHorarios.forEach(h => {
            ...
            const desdeDate = new Date(h.vigente_desde);
            const hastaDate = h.vigente_hasta ? new Date(h.vigente_hasta) : null;
            const desdeStr = !isNaN(desdeDate.getTime()) ? dateFormatter.format(desdeDate) : 'Fecha Inválida';
            const hastaStr = hastaDate && !isNaN(hastaDate.getTime()) ? dateFormatter.format(hastaDate) : 'Fecha Inválida';
            const vigencia = `Desde: ${desdeStr} ${h.vigente_hasta ? `Hasta: ${hastaStr}` : ' (Indefinido)'}`;
