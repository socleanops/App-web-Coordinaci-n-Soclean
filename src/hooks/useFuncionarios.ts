    ...
    const createFuncionario = useMutation({
        mutationFn: async (formData: FuncionarioFormData) => {
            ...
            const processPromise = (async () => {
                ...
                let safePassword = formData.password?.trim();
                let isGeneratedPassword = false;
                if (!safePassword) {
                    safePassword = generateComplexPassword();
                ...
                if (isGeneratedPassword) {
                    toast.success(`5/5 Operación exitosa! Contraseña temporal generada: ${safePassword}`, { id: tid, duration: 10000 });
                } else {
